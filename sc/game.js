...
(function(){
	...
	var mainState = {
		...
		//input variables to create the game activities.
		create : function(){
			this.lastBullet = 0;
			this.lastEnemy = 0;
			this.lastTick = 0;
			...
			//set game background (having image multiply across screen)
		...
		update : function(){
			...
			if(curTime - this.lastTick > 10000)
			{
				if(this.speed < 500)
				{
					...
					this.bg2.autoScroll(-this.bg2Speed, 0);
					this.bg3.autoScroll(-this.bg3Speed, 0);
					this.lastTick = curTime;// resets to current time.
				}
			}
			//collision.
		...
		generateEnemy : function(){
			...
			if(enemy)
			{
				enemy.reset(WIDTH - 800,Math.floor(Math.random()*(HEIGHT-30)),'enemyship'+(1+Math.floor(Math.random()*5)));
			}
			else
			{
				enemy = this.enemies.create(WIDTH - 00,Math.floor(Math.random()*(HEIGHT-30)),'enemyship'+(1+Math.floor(Math.random()*5)));
			}
